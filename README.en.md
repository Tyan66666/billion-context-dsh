...
| `decompress` | Restore a previously compressed block's original content (read-only); accepts the `bN` ref shown by acp_status or a compaction id. Large blocks are paged (default 100 messages per call): pass `offset`/`limit` and follow the continue hint in the result, so one recovery can never flood the context or get silently trimmed by the host |
