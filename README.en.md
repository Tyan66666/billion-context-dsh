...
- **See the active tier**: `/acp status` prints the effective `preset` and the three resolved thresholds (including any explicit override you made on top of it). Note that this line reads the composition-time `env` values — a same-name key in `coreOverrides.nudge` lands inside `kernelConfigFor` and is **not** reflected here, so the printed numbers can be lower than the ones actually in force.
