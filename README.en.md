...
The load-bearing compression guidance (tools, philosophy, summary rules, tier rules, **provider prefix-cache knowledge** — compressing shifts every later message's position and invalidates that span's suffix cache once: compressing the tail is nearly lossless, a large mid-context block (e.g. an old injection row) costs the most, engine metadata folding into a real span costs nothing extra) is registered as a one-time system-prompt section; each nudge carries a condensed version (efficiency note + philosophy + context breakdown + HOW_TO_COMPRESS_RULES + range table + batch tip). There is deliberately **no automatic summarization**: automatic policy only nudges the model (`compactIfNeeded` returns null).
