...
8. **Successful compress call/result pairs MUST be hidden; orphan tool messages MUST be pruned before range solving (but never the in-flight compress call)** — the durable summary node is written mid-turn, before the current `compress` tool/result lands, so leaving that pair visible produces `assistant(tool_calls) → user(summary) → tool(result)`, which strict providers reject with HTTP 400. The engine hides the pair after `session/event` sees its `tool/result` by replacing the two tool nodes with a plain user message that preserves the result text. Separately, one orphan `tool/result` corrupts the pairing balance cache and an orphan tool-call fragments large ranges; `buildCompressibleSeqRanges` and `handleCompress` strip them via the durable `compaction/prune` protocol before any range solve. `handleCompress` passes the current call id to the stripper so the executing compress call is never pruned before its result lands.
...
4. **The test suite is the safety net**: 84 tests cover the battle-hardened behaviors (CJK estimation, lone-tool expansion, surface range table, ledger backfill, ref-tag projection, orphan-tool pruning, compress call/result hiding, in-flight call preservation). Any kernel behavior change that breaks one of those turns red here — do NOT release on red.
