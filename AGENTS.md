...
3. **Nudge range table MUST be computed from the surface** (`buildCompressibleSeqRanges`), NOT from kernel `compressibleRanges` — the kernel ref map drifts after surface replacements in long sessions, hiding large tool results and producing `end < start` ranges. Ranges are ordered **oldest-first** (stable across turns — the model can consume them front-to-back without re-ranking; matches the kernel's `oldest first` list and the host's compression rhythm) and each row carries a **kernel-parity tool/text share** (`[tool X% | text Y%]`, count-based `toolPct` via `isToolEvent` — tool-call + tool-result both count as tool) so the model can spot consumed tool-heavy spans without a drilldown.
