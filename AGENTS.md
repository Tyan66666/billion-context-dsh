...
4. **The test suite is the safety net**: 104 tests cover the battle-hardened behaviors (CJK estimation, lone-tool expansion, surface range table, ledger backfill, ref-tag projection, orphan-tool pruning, broken-pair healing, deferred compress call/result hiding, in-flight call preservation, batch resilience, acp_status upstream alignment, dual-id bN decompress, tool-result toolName backfill, acp_status drilldown passthrough + mN/seq separation + checkpoint row exclusion + multi-call survival). Any kernel behavior change that breaks one of those turns red here — do NOT release on red.
