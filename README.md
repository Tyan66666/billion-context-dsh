...
**（可选）自定义提示词文案 —— `config.prompts`。** 所有模型可见的提示词（普通/紧急 nudge 首句、上下文分解、增长行、批量提示、tier 蒸馏行、范围表、ACP system prompt 段、四个工具描述）默认**直接复用 acp-kernel 的 `renderNudgeText`**——效率提示、上下文分解、压缩规则、批量提示全部来自 kernel 原文，仅范围表换成 surface-seq 版（kernel 用 mNNNNN 引用，我们架构没有 `<acp>` 标签；seq 范围表同样携带 `[tool X% | text Y%]` 组成占比并 oldest-first 排序，与 kernel 展示语义一致）。范围表按真实用户消息分段：引擎自己的元数据行（acp-index 目录行、nudge 回显、压缩占位行）**并入相邻可压段**——压已消费内容时顺手带走（tokens 计入显示、`tool` 占比按真实消息算）；**当前生效的指令注入**（最新版 AGENTS.md 全文、技能目录、系统策略快照）与 checkpoint **不可压**，永不进范围表——但**被新版本取代的 AGENTS.md 旧副本**可随相邻段压缩回收（宿主只看当前版本是否可见，压旧副本不会触发重新注入；实测长会话里旧副本可占表面 40%+）。覆盖任一 nudge 槽位后自动切换到模板渲染。模板支持命名占位符（如 nudge 的 `{pct}`、`{philosophy}`、范围表的 `{surface}`），**构造期校验**：占位符拼写错误会在引擎启动时抛错（fail-fast），而不是把字面 `{pct}` 漏进模型上下文：
...
为上一条索引之后新出现的每个 surface 节点标注 seq、类型与内容预览（token 预算截断，默认 16 tokens，CJK 按 1 字/token 计）。**超过 512 tokens 的大节点会附 `[N tok]` 标记**（千位以上显示为 `[1.5K tok]`），让模型一眼认出「谁是回收重点」——未标注只表示低于阈值，不表示空内容，精确大小可随时经 `acp_status` 钻取。**宿主指令/策略行（AGENTS.md 注入、技能目录、运行时策略快照，标 `instr`）与引擎回显行（nudge 回显等，标 `meta`）只编号不给预览**——其全文本本就躺在上下文里，目录只负责把 seq 对回去；`[N tok]` 标记保留，大块策略行仍以策略行的身份可见。目录**每回合注入一次**（模型内部工具续步不重复注入）；**若一次超长回合（单次发言后连续几十个工具步）让未编号的*工具文本*（工具结果内容 + 工具调用参数，助手自己写的散文不计）每累计 ~8K tokens（`maxDelayToolTokens` 默认 8192）就在工具续步中自动补发一次目录行**，保证轮内对位不失真（对话消息默认不参与推动——`maxDelayTextTokens` 默认 `0`，需要时单独设阈值；短回合不触发，行为与纯每回合一致）。触发回合的用户消息要等下一回合才编号（追加式日志的固有顺序）。模型由此随时把任意 seq 对回它看过的内容——压缩范围表、`acp_status` 钻取行、`compress` 边界从此都有文本锚点。压缩后旧索引行随被遮蔽的原文一起消失（定位交给块摘要，且不再进入 `search_context` 文档集），新行从最新 live seq 继续编号，id 空间保持连续。冷启动接入存量会话时，待编号消息超过 `backlogLimit` 会改发一行占位目录（只列 seq 范围），避免单条消息吞下全部积压——占位文案明确指引归档对象是**被列出的 seq 区间**而非占位行自身（单行过不了内核 5000 字符压缩门槛）：
...
```yaml
      config:
        messageIndex:
          ...
          maxDelayToolTokens: 8192 # 长轮补发阈值：未编号工具输出累计达到即补发目录行；0 关闭
          maxDelayTextTokens: 0    # 对话消息累计阈值：0 = 对话不参与推动（需要时单独设）
```
...
| 每消息编号（pi 的内联 `<acp>` 标签） | 持久化 `[acp-index]` 插件消息：每回合一次追加单行目录（长轮中未编号工具输出越过 `maxDelayToolTokens` 会提前补发），为新 surface 节点标注 seq + 类型 + 预览 |
...
承载性的压缩指引（工具、哲学、摘要规则、tier 蒸馏/浓缩规则、**提供者前缀缓存知识**——压缩使被压位置之后的所有消息位置偏移、该段之后缓存一次性失效：压尾部近无损、中部大块（如旧的 AGENTS.md 副本）代价最高、引擎元数据与旧指令副本随真实段搭车零额外代价）注册为一次性系统提示段；每条 nudge 携带精简版（效率提示 + 哲学 + 上下文分解 + 压缩规则 + 范围表 + 批量提示）。刻意**不做自动摘要**：自动策略只 nudge 模型（`compactIfNeeded` 返回 null）。
...
| `messageIndex` | `{ enabled: false, previewTokens: 16, backlogLimit: 100, maxDelayToolTokens: 8192, maxDelayTextTokens: 0 }` | 每消息编号索引（早期版本默认关闭，手工开启）：每回合一次在 pre-step 注入单行 `[acp-index]` 目录消息，为新 surface 节点标注 seq + 类型 + token 预算预览，让模型把任意 seq 对回看过的内容；长轮中未编号**工具输出**累计超过 `maxDelayToolTokens` 会提前补发目录行（`0` 关闭），**对话消息**累计超过 `maxDelayTextTokens` 同样触发（默认 `0` = 对话不参与推动）；积压超过 `backlogLimit` 时改发占位行。`enabled: true` 开启。见 [docs/message-index-design.md](docs/message-index-design.md) |
...
├── message-index.ts# M6: acp-index 每消息编号目录（日志水位 + token 预算预览 + 长轮 token 延迟补发）
