...
承载性的压缩指引（工具、哲学、摘要规则、tier 蒸馏/浓缩规则、**提供者前缀缓存知识**——压缩使被压位置之后的所有消息位置偏移、该段之后缓存一次性失效：压尾部近无损、中部大块（如旧注入行）代价最高、引擎元数据随真实段搭车零额外代价）注册为一次性系统提示段；每条 nudge 携带精简版（效率提示 + 哲学 + 上下文分解 + 压缩规则 + 范围表 + 批量提示）。刻意**不做自动摘要**：自动策略只 nudge 模型（`compactIfNeeded` 返回 null）。
