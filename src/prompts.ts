...
- acp_status: current context usage and the live compressible-range list. Run it right before compressing — the only seqs that never go stale are the ones you just read; if you are unsure about context pressure or block structure, drill in first (scope/view/tool/sort/limit list per-message or per-block sizes; drilldown rows are kernel ids (mN) — compress accepts them directly, auto-mapped to the live surface seq).
