...
function buildSearchDocs(session: Session): SearchDoc[] {
  ...
  for (const block of ledger) {
    ...
    for (const seq of expandShadowedSeqs(session, block.blockId)) {
      ...
      // The engine's OWN metadata rows (acp-index directory lines, nudge
      // echoes, compress-pair stubs) and host instruction/policy rows
      // (AGENTS.md injections, skill catalogs, system-prompt snapshots) are
      // synthetic or durable text. As shadowed docs they carry long text in
      // the user role, and the kernel's role weighting (user 1.5× vs tool
      // 0.6×) let them monopolize the top-5 for common queries — real hits
      // never made the leaderboard. Composing the doc set is adapter work;
      // ranking stays kernel-owned (rules 6/7 untouched).
