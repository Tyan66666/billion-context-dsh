...
- 投影路径（`projectEvent`）：对 checkpoint 节点（`isCheckpointNode`：`user/message` 且双形状 checkpoint source——legacy `plugin:'compact'` 或 0.1.7 `kind:'compact-checkpoint'`，issue #166）补框——幂等，仅为**升级前的旧块**兜底。新块到达投影时已带框，`withSummaryFramePrefix` 直接返回原文。
