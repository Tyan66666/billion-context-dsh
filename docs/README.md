...
> **⚠️ Beta** — this project (v0.2.25) and the [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) are both in **public beta**: do not use in engineering / production environments.
...
| [Presets design](presets-design.md) | One-word nudge aggressiveness (`config.preset`): five tiers (preserve / relaxed / balanced / efficient / aggressive) resolved into the three nudge thresholds, precedence explicit value > preset > default, display-only in `/acp-prune status`; composition-layer today, settings hot-swap rides on #75 |
...
| [Runtime settings integration design](settings-integration-design.md) | Phase-1 settings seam (issue #75): the six scalar knobs hot-editable via `~/.dsh/settings.yaml` / `/acp-prune config`, layering (schema default → composition row → user section), base-filtering rationale, window-cache invalidation, kill switch, /acp-prune config parse rules |
...
| [Overflow recovery design](overflow-recovery-design.md) | Why the engine performs exactly ONE automatic compaction — a provider-confirmed context overflow (`CONTEXT_WINDOW_EXCEEDED`) hides the largest eligible range with an engine-written marker summary and answers `agent/request-error` with `{kind:'retry'}`, while the pressure side stays nudge-only; the `maxOverflowRetries` budget (default 1, `0` disables), why the marker is exempt from `SUMMARY_FRAME_PREFIX` by CONTENT rather than a caller flag, and the known limits |
