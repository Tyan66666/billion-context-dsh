...
>
> - **R7**:默认模板追加"活性感知"压缩引导(见 issue #149:模型把进行中的 span 压成"结论报告",丢掉任务状态——走到哪/待办、已排除的死路、已就绪产物的位置——导致压缩后重跑目录搜索/API 校验/npm pack):① `rangeTable.footer` 追加一句行动区自检「Before you compress any row above, confirm it is truly consumed — if a span still holds live task state (where you are in the task / open TODOs, paths already ruled out, artifacts already done and where they live), carry that state into your summary or keep the span live. Compressing an in-progress span without carrying its state forces a full re-run afterward.」——放在范围表正下方(nudge 里动作点最近的位置);因 B6 瘦身后 nudge 正文不再带完整规则段,这条是模型动手前最贴近的提示。② 系统提示 summary-writing 段补一句任务态交接「A mid-task compression must also hand off STATE, not just facts: where you are in the task and what remains (open TODOs), which approaches you already ruled out (dead ends), and which intermediate artifacts are already done and where they live — so the next step continues instead of re-running work you already did.」。说明:kernel 的 `HOW_TO_COMPRESS_RULES` 本就要求保留 TODO/死路/路径(R6 之前即有),"要保留什么"并不缺;本条是把这两处**引擎自有**文案补齐到同一语义(效率框架下别把进行中的场景当死输出压掉),不改 kernel。改宿主默认模板,不 bump kernel。回归:prompts 测试 #5e 钉住两处语义载体,测试 #4 范围表字节快照同步更新。
...
```
> 注:示例保持与 `src/prompts.ts` `DEFAULT_PROMPTS.tools.*` 逐字同步(本 PR 同步了 searchContext 强化版并顺带对齐了 decompress/acpStatus 两个此前过期的示例行)。
