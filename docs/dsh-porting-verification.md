...
> **宿主注入源契约（注入行分层的前提）**——`classifySurfaceEvent`（src/messages.ts）按 plugin 名白名单（acp-index/acp-nudge/billion-context-dsh = metadata 可折叠）与指令清单（agent-instructions 两种形态/skill-catalog/@deepseek-ai/dsh-system-prompt/tool-jobs/runtime-context = instruction 不可压）分类；**任何未知名 `kind==='plugin'` 行保守落 instruction（不可压）**。契约：宿主新增注入源时，必须同步 `classifySurfaceEvent` 并补 fixture——否则新注入形态会被误判进可压段（v1 标记泄漏问题复发）。当前宿主形态枚举见 docs/injection-compressibility-design.md。
