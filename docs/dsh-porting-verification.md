...
> **`UPSTREAM:` dsh-compaction@0.1.2-rc.1 工具配对平衡缓存读取已删除的 `session.events`（AGENTS.md rule 11 / issue #124）**——宿主 `toolPairingBalancedBefore/After` 的 `extendCache` 读 `session.events`（`eventForSeq` 做 `events[seq]`），而 `dsh-session@0.1.2-rc.1` 已删除 `events` getter（改为 `snapshotEvents()`/`eventAt(seq)`）→ 在 0.1.2 宿主上一切压缩必炸 `TypeError: Cannot read properties of undefined (reading '<seq>')`，包括宿主自带的 `dsh-compaction-basic`（其 `lib/index.js:395/527/528` 调用同一组 helper）。引擎现用本地镜像 `src/tool-pairing.ts`（算法逐行复刻宿主 `tool-pairing` 模块，唯一差异是事件读取走跨版本 `eventAtOf`，src/session-events.ts），`src/region.ts` 不再 import 宿主 helper。**当宿主修复发布（dsh-compaction 改读 `snapshotEvents()`/`eventAt()`）且 peer 范围覆盖该版本时，删除 `src/tool-pairing.ts` 并恢复宿主 helper**；回归测试 `tests/tool-pairing-host-012.test.ts` 守护三件事：rc.6 上与宿主 helper 对拍一致、0.1.2 形状（无 `events`）下宿主 helper 崩溃而镜像正常、`UPSTREAM:` 标签与 import 替换不被静默回退。当前上游状态：未修复（2026-09-06 发现于宿主 0.1.2-rc.1；离线取证：用宿主自身 `decodeStorageRecord` + `Session.fromRestore` 恢复 104,421 事件后调用 `resolveSurfaceRange` 精确复现，取证要点见 issue #124）。

> **`UPSTREAM:` nudge 上下文 breakdown 口径：全日志 vs 活跃 surface（AGENTS.md rule 2）**——`buildNudge` 为支持 T2/T3 蒸馏锚点，必须把**整个日志**（`allLogMessages`，含已被压缩进 block 的历史消息）喂给 `kernel.processTurn`；kernel 的 `computeContextBreakdown` 对这份消息数组分类加总，于是 nudge 展示带上**历史累积**口径——实测 nudge 报 `85.2K tool`，而 acp_status 报真实活跃 `8.5K tool`，相差约 10 倍（核查记录见 /workspace/context-check-85.2K-tool.md）。`acp_status` 用 `buildStatusReport` 喂**活跃 surface**（排除 `source.plugin==='compact'` 的 checkpoint 摘要节点——《`acp` status 的 `isCheckpointEvent`），所以它反映当前真实上下文。修复：`src/nudge.ts` 导出 `computeSurfaceBreakdown(state, messages, total, growth)`——复刻 kernel 分类（tool-call/tool-result→tool、role system→system、含```→code、否则→text），但 **summaries 直接取 active blocks 的 `block.summary`**（kernel 源码靠消息文本 `[Compressed conversation section]` 前缀识别摘要，而 DSH checkpoint 节点从不带此前缀，故须读 block 而非消息文本）；`buildNudge` 用活跃 surface（排除 checkpoint 节点）调用它覆盖 `nudge.contextBreakdown`。两条渲染路径（kernel `renderNudgeText` + 模板 `renderNudgeFromTemplates`）都读 `nudge.contextBreakdown`，覆盖一次即统一。该字段纯展示、不参与 `shouldInject` 决策，覆盖安全。回归测试 tests/nudge.test.ts（活跃 surface tool < 全日志 tool + block summaries>0；无块形状校验）。测试易踩坑：`compress` 工具的 `resolveSurfaceRange` pass-2 会把相邻 tool 对一并扩展吃掉（`adjusted from 2..3 to balanced edges` 扩到 1..4），若测试要保留 surface tool，须用 `runCompactionTransaction` 直接精确压范围而非走 compress 工具。

> **`UPSTREAM:` 宿主 0.1.5-alpha.1 重命名 `SurfaceOp` 替换字段（AGENTS.md rule 13）**——DSH（host commit `657e68186a`）把命令式的位置替换从 `{ op:'replace', start, end }` 改成 `{ op:'replace', startSeq, endSeq }`，且宿主 `surface.ts` 的 `isReplaceOp`（0.1.5+）现在**严格拒绝**任何非"恰好 `op`+`startSeq`+`endSeq` 三键"的形状；`planSurfaceEvent` 在**会话 replay/恢复折叠**时也跑同一校验，所以旧字段事件在 0.1.5 宿主上不只是新压缩抛错，**历史 replay 同样抛 `session event "<type>" carries an invalid replace surfaceOp`**。引擎旧代码写 `{ start, end }` → 升级 dsh 本体到 0.1.5 后每次压缩/恢复都报此错。修复：`src/surface-op.ts` 版本感知——`createRequire` 探测**字面安装的** `@deepseek-ai/dsh-session/package.json` 版本（`./package.json` 是子导出），`usesNewSurfaceOpShape` 阈值 `SURFACE_OP_RENAME_VERSION = '0.1.5-alpha.1'`（≤0.1.3-alpha.2 旧、≥0.1.5-alpha.1 新，0.1.4 从未发布），进程内缓存（`installedUsesNewSurfaceOpShape`）；纯函数核 `replaceSurfaceOp`/`compareSurfaceOpVersions`（自包含 pre-release-aware 比较，不引入运行时 `semver`——它是 devDeps 不进 bundle）。`region.ts` 三处写入点（runCompactionTransaction 的摘要节点、hideSurfaceSeqs 的 user/assistant 节点）统一改走 `installedSurfaceReplaceOp`，`/acp compress` 经事务自动覆盖。类型承认点集中在 `InstalledReplaceSurfaceOp = Extract<SurfaceOp,{op:'replace'}>`（本地 0.1.0-rc.6 类型只拼旧字段，运行时探测保证字节匹配宿主）。peer range 加了 `^0.1.5-alpha.1` 子句（`tests/peer-range.test.ts` 断言 0.1.5 线可安装、0.1.3/0.2.x 仍拒绝）。回归测试 `tests/surface-op.test.ts` 覆盖新旧两线字段名、改名边界、alpha 数字标识比较。当前状态：修复已落地。
