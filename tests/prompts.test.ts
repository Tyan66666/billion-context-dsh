...
test('M4/prompts 5: tool descriptions render the defaults byte-identical', () => {
  ...
  assert.equal(descriptions['decompress'], 'Recover the original content of a compressed block by its blockId — the kernel block ref `bN` shown by acp_status (e.g. b1), or a compaction id from search_context (read-only; does not unshadow the range). Large blocks are paged (default 100 messages per call): pass offset/limit to walk them and follow the continue hint in the result.')
