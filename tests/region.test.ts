...
test('M5: the checkpoint replace op speaks the current SurfaceOp protocol (startSeq/endSeq, issue #136)', () => {
  // dsh-session >= 0.1.3-alpha.2 validates replace surfaceOps as EXACTLY
  // `{ op, startSeq, endSeq }` (deepseek-harness commit 27bf1039db renamed
  // `start`/`end`); emitting the old field names makes every compress call
  // fail with "session event ... carries an invalid replace surfaceOp". The
  // devDep session IS the validator here — an old-shape op would throw at
  // append time and this test would fail before its assertions run.
  const session = buildTextSession(6)
  const { seqs } = runCompactionTransaction(session, {
    start: 1,
    end: 4,
    shadowedSeqs: [1, 2, 3, 4],
    summary: [{ type: 'text', text: 'A summary of the range with enough detail.' }],
    shadowedTokenCount: 123,
    provider: 'test-provider',
    model: 'test-model',
  })
  const checkpoint = session.snapshotEvents()[seqs[2]!]!
  assert.equal(checkpoint.type, 'user/message')
  const surfaceOp = (checkpoint as { surfaceOp?: unknown }).surfaceOp
  assert.deepEqual(surfaceOp, { op: 'replace', startSeq: 1, endSeq: 4 })
  assert.equal(Object.keys(surfaceOp as object).length, 3, 'the new protocol requires EXACTLY three keys (old + new names together are rejected)')
  assert.equal('start' in (surfaceOp as object), false, 'the old start/end names must be gone')
  assert.equal('end' in (surfaceOp as object), false, 'the old start/end names must be gone')
})
...
test('M5: stripOrphanedSurfaceToolMessages removes orphan results and orphan calls', () => {
  ...
  // Each pruned seq is replaced on the surface by a terse user message — the
  // only replace idiom dsh-session >= 0.1.3 accepts for a shadowed range (an
  // empty assistant replace is rejected on replay: sourceEventSeqs are
  // forbidden on assistant/message, so the shadowed nodes could never be
  // cited — see hideSurfaceSeqs). The originals stay recoverable in the log.
  const messages = session.deriveMessages()
  assert.equal(messages.length, 4, 'q0, q1, and one placeholder user message per pruned seq')
  const texts = messages.map((m) => (m as { content?: { type: string; text?: string }[] }).content?.map((b) => b.text).join('') ?? '')
  assert.equal(texts.filter((t) => t === '(pruned tool message)').length, 2, 'the placeholders name the prune')
  assert.ok(texts.includes(longText('q0', 0)) && texts.includes(longText('q1', 1)), 'the real messages survive')
...
test('M5: stripOrphanedSurfaceToolMessages prunes legacy broken pairs (call → summary → result)', () => {
  ...
  const after = session.deriveMessages()
  assert.equal(after.length, 4, 'the two real user messages plus one placeholder user message per pruned seq')
